...
class Move(models.Model):
    ...
    def submit_invoice(self):
        if self.send_electronic_invoice == False:
            ...
            url = base_url + "/API/Receipt/ImportReceipt"
            access = self.login()
    ...
    def login(self):
        base_url = self.get_base_url()
        url = base_url + "/api/account/GetToken"
        # clientId, clientSecret = self.get_clientId_Secret()
        headers = {'content-type': "application/x-www-form-urlencoded", 'cache-control': "no-cache"}
        payload = {
            'ProductIdentity': "e2daf8aa-9bcf-4d88-9133-b067fbc72b84",
            'ProductSecretKey': "2adac66a-6b9e-4399-a324-a9cc248844a0",
        }
