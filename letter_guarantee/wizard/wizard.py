...
class ExtendAmountWizard(models.TransientModel):
    _name = 'amount.wizard.log'

    name = fields.Char("Description")
    amount = fields.Float("Amount")
    letter_credit = fields.Many2one('letter.guarantee')

    def calculate_amount(self):
        amount = self.amount
        rate = self.letter_credit.curr_rate
        result = amount / rate
        return result

    def confirm_action(self):
        today = datetime.datetime.now()
        lc = self.letter_credit
        lc.lc_remaining_amount += self.amount
        amount_fees = (lc.lc_type.bank_fees / 100) * self.calculate_amount()
        lc.lc_amount += self.amount
        # Create the 3 journal Entries records
        lc_bank_journal = self.env['account.move'].sudo().create({
            'lc_seq': lc.name,
            'date': today,
            'journal_id': lc.lc_type.lc_bank_journal.id,
            'currency_id': lc.currency_id.id
        })
        lc_journal = self.env['account.move'].sudo().create({
            'lc_seq': lc.name,
            'date': today,
            'journal_id': lc.lc_type.lc_journal.id,
            'currency_id': lc.currency_id.id
        })
        # print(lc_bank_journal.id)
        # print(lc_journal.id)
        if lc.lc_type.bank_fees_from_lc:
            lc.lc_remaining_amount -= (amount_fees * lc.curr_rate)
            ...
            debit3 = {'move_id': lc_journal_fees_true.id,
                      'account_id': lc.lc_type.bank_expense_account.id,
            ...
            credit3 = {'move_id': lc_journal_fees_true.id,
                       'account_id': lc.lc_type.lc_journal.default_account_id.id,
                       'partner_id': lc.partner_id.id,
                       'date': today,
                       'currency_id': lc.currency_id.id,
                       'debit': 0.0,
        ...
        elif not lc.lc_type.bank_fees_from_lc:
            ...
            credit2 = {'move_id': lc_bank_journal_fees_false.id,
                       'account_id': lc.lc_type.lc_bank_journal.default_account_id.id,
                       'partner_id': lc.partner_id.id,
                       'date': today,
                       'currency_id': lc.currency_id.id,
                       'debit': 0.0,
        ...
        debit = {'move_id': lc_bank_journal.id,
                 'account_id': lc.lc_type.intermediate_account.id,
                 'partner_id': lc.partner_id.id,
                 'date': today,
                 'currency_id': lc.currency_id.id,
        ...
        self.env['lc.amount.extend'].sudo().create({
            'lc_seq': lc.name,
            'name': self.name,
            'amount': self.amount
        })
...
class ExtendPeriodWizard(models.TransientModel):
    _name = 'period.wizard.log'

    name = fields.Char("Description")
    date = fields.Datetime("Expiration Date")
    letter_credit = fields.Many2one('letter.guarantee')
    letter_guarantee_type = fields.Many2one('letter.guarantee.type', string="LOG Type")

    def confirm_action(self):
        lc = self.letter_credit
        log_type = self.letter_guarantee_type
        lc.expiration_date = self.date
        today = datetime.datetime.now()

        self.env['lc.period.extend'].sudo().create({
            'lc_seq': lc.name,
            'name': self.name,
            'date': self.date
        })

        amount_fees = (log_type.bank_fees / 100) * lc.log_remaining_amount
        if log_type.bank_fees_from_lc:
            lc.log_remaining_amount -= amount_fees
            lc_journal_fees_true = self.env['account.move'].sudo().create({
                'lc_seq': lc.name,
                'date': today,
                'journal_id': log_type.lc_journal.id,
                'currency_id': lc.currency_id.id
            })

            debit3 = {'move_id': lc_journal_fees_true.id,
                      'account_id': log_type.bank_expense_account.id,
                      'partner_id': lc.partner_id.id,
                      'date': today,
                      'currency_id': lc.currency_id.id,
                      'debit': amount_fees,
                      ...
                      'analytic_account_id': False}
            credit3 = {'move_id': lc_journal_fees_true.id,
                       'account_id': log_type.lc_journal.default_account_id.id,
                       'partner_id': lc.partner_id.id,
                       'date': today,
                       'currency_id': lc.currency_id.id,
                       'debit': 0.0,
            ...
            lc_journal_fees_true.write({'line_ids': [(0, 0, debit3), (0, 0, credit3)]})
        elif not log_type.bank_fees_from_lc:
            lc_bank_journal_fees_false = self.env['account.move'].sudo().create({
                'lc_seq': lc.name,
                'date': today,
                'journal_id': log_type.lc_bank_journal.id,
                'currency_id': lc.currency_id.id
            })
            ...
            debit2 = {'move_id': lc_bank_journal_fees_false.id,
                      ...
                      'analytic_account_id': False}
            credit2 = {'move_id': lc_bank_journal_fees_false.id,
                       'account_id': log_type.lc_bank_journal.default_account_id.id,
                       'partner_id': lc.partner_id.id,
                       'date': today,
                       'currency_id': lc.currency_id.id,
                       'debit': 0.0,
...
class CutOfLog(models.TransientModel):
    _name = 'cut.log'
    ...
    def calculate_amount(self):
        lc = self.letter_credit
        amount = self.amount * (lc.lc_type.log_cover_percentage / 100)
        rate = self.letter_credit.curr_rate
        result = amount / rate
        return result
    ...
    def confirm_action(self):
        today = datetime.datetime.now()
        lc = self.letter_credit
        lc.log_remaining_amount -= self.amount
        lc.lc_remaining_amount -= (lc.lc_type.log_cover_percentage / 100) * self.amount
...
#     _name = 'account.payment.register.lc'
#
#     letter_credit = fields.Many2one("letter.guarantee", string="LOG")
#     # == Business fields ==
...
#     line_ids = fields.Many2many('account.move.line', 'account_payment_register_move_line_inherit_rel', 'wizard_id',
#                                 'line_id',
...
#                   'analytic_account_id': False}
#
#         lc_journal.write({'line_ids': [(0, 0, debit), (0, 0, credit)]})
#
...
class AccountPaymentLOG(models.Model):
    _name = 'account.payment.log'

    letter_credit = fields.Many2one("letter.guarantee", string="LOG")
    journal_id = fields.Selection([('bank', 'Bank'), ('cash', 'Cash')], default='bank')
    payment_type = fields.Selection([
        ('outbound', 'Send Money'),
        ('inbound', 'Receive Money'),
    ], string='Payment Type', default='outbound', required=True)
    payment_method = fields.Selection([('manual', 'Manual'), ('checks', 'Checks')], default='manual')
    partner_type = fields.Selection([
        ('customer', 'Customer'),
        ('supplier', 'Vendor'),
    ...
    def confirm_action(self):
        ...
        credit = {'move_id': lc_journal.id,
                  ...
                  'analytic_account_id': False}
        debit = {'move_id': lc_journal.id,
                 'account_id': lc.partner_id.property_account_receivable_id.id,
                 'partner_id': lc.partner_id.id,
                 'date': today,
                 'currency_id': lc.currency_id.id,
                 'debit': self.calculate_amount(),
        ...
        lc_journal.write({'line_ids': [(0, 0, credit), (0, 0, debit)]})
